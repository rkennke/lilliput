...
uintptr_t SlidingForwarding::encode_forwarding(HeapWord* from, HeapWord* to) {
  ...
  if (to_region_base <= to && to < to_region_base + BLOCK_SIZE_WORDS) {
    // Primary is good
    ...
    *base = to;
    ...
    to_region_base = *base;
    if (*base <= to && to < *base + BLOCK_SIZE_WORDS) {
      // Alternate is good
    } else if (*base == UNUSED_BASE) {
      // Alternate is free
      *base = to;
...
inline void SlidingForwarding::forward_to_impl(oop from, oop to) {
  ...
  assert((!UseCompactObjectHeaders) || new_header.narrow_klass() != 0, "null narrowKlass: " INTPTR_FORMAT, new_header.value());
