...
uintptr_t FullGCForwarding::encode_forwarding(HeapWord* from, HeapWord* to) {
  ...
  if (to_region_base == UNUSED_BASE) {
    // Base at -1, to support offset=0 as placeholder for 'not forwarded'.
    _biased_bases[from_block_idx] = to_region_base = to - 1;
  }
  assert(to_region_base < to, "base address must be below forwarding address");
  ...
  assert((encoded & ~OFFSET_MASK) == 0, "must encode to available bits");
  return encoded;
}
...
HeapWord* FullGCForwarding::decode_forwarding(HeapWord* from, uintptr_t encoded) {
  ...
  assert((encoded & ~OFFSET_MASK) == 0, "must decode from available bits, encoded: " INTPTR_FORMAT, encoded);
