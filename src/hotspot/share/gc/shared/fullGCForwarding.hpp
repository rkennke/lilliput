 ...
 *   2. Load the base-offset of the from-block from the side-table.
 ...
 *
 * We reserve two special values for the offset:
 *  - 00000000000: Indicates that the object has not yet been forwarded.
 *  - 11111111111: Indicates an exceptional forwarding (see below), for which a fallback hash-table
 *                 is used to look up the target address.
 *
 * In order to support this, we need to make two changes to the above algorithm:
 *  - The base addresses that are stored in the side-table have to be first-to-address - 1, this way
 *    all valid forwardings for the block would start at offset 1.
 *  - Forwardings that would use offsets >= 11111111111 (i.e. the last two slots, because of the shift by 1)
 *    would also need to use the fallback-table. We expect that to be relatively rare for two reasons:
 *    1. It only affects 2 out of 2048 possible offsets, in other words, 1/1024th of all situations in an equal
 *       distribution.
 *    2. Forwardings are not equal-distributed, because often we would 'skip' unreachable objects. Forwardings
 *       tend to cluster at the beginning of the target region, and become less likely towards the end
 *       of the possible encodable target address range. Which means in reality it will be much less frequent
 *       than 1/1024.
 *
 ...
 *   need to be forwarded to a different address than earlier objects in the same block. This is rare.
...
private:
  static const int AVAILABLE_LOWER_BITS       = 11;
  ...
  static const int NUM_OFFSET_BITS = AVAILABLE_LOWER_BITS - OFFSET_BITS_SHIFT;
