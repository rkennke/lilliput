...
MAYBE_INLINE_EVACUATION
void G1ParScanThreadState::do_partial_array(oop obj, int chunk, int pow) {
  ...
  while ((1 << pow) > (int)ObjArrayMarkingStride && (chunk*2 < G1TaskQueueEntry::chunk_size())) {
    pow--;
    chunk *= 2;
    push_on_queue(G1TaskQueueEntry(array, chunk - 1, pow));
  }
  ...
  if (len <= (int) ObjArrayMarkingStride*2) {
    ...
    while ((1 << pow) > (int)ObjArrayMarkingStride &&
      ...
      if (left_chunk_end < len) {
        push_on_queue(G1TaskQueueEntry(array, left_chunk, pow));
        chunk = right_chunk;
...
void G1ParScanThreadState::dispatch_task(G1TaskQueueEntry task) {
  ...
  if (task.is_narrow_oop_ptr()) {
    ...
    do_partial_array(task.to_oop(), task.chunk(), task.pow());
  }
}
